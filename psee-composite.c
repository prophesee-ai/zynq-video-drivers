...
/**
 * psee_graph_entity_set_streaming - Update the streaming status
 * @pdev: Composite video device
 ...
 */
static bool psee_graph_entity_set_streaming(struct psee_composite_device *pdev,
					    struct psee_graph_entity *entity,
					    bool enable)
{
...
static int
psee_graph_entity_start_stop_subdev(struct psee_composite_device *pdev,
				    struct psee_graph_entity *entity, bool on)
{
	struct v4l2_subdev *subdev;
	int ret = 0;

	dev_dbg(pdev->dev, "%s entity %s\n",
	...
	if (on) {
		...
		if (ret < 0 && ret != -ENOIOCTLCMD) {
			...
			psee_graph_entity_set_streaming(pdev, entity, 0);
			return ret;
		}
		...
		if (ret < 0 && ret != -ENOIOCTLCMD) {
			...
			psee_graph_entity_set_streaming(pdev, entity, 1);
		}
...
/**
 * psee_graph_entity_start_stop - start / stop the graph entity
 * @pdev: composite device
 ...
 */
static bool psee_graph_entity_start_stop(struct psee_composite_device *pdev,
					 struct psee_graph_entity *entity,
					 bool on)
{
	...
	for (i = 0; i < entity->entity->num_pads; i++) {
		struct psee_graph_entity *remote;
		 ...
		 */
		remote = psee_graph_find_entity_from_media(pdev, pad->entity);
		if (!remote)
			continue;

		/* the dependency state doesn't meet */
		if (remote->streaming != on) {
			state = psee_graph_entity_start_stop(pdev, remote, on);
			if (!state)
				return state;
		}
	}
	...
	state = psee_graph_entity_set_streaming(pdev, entity, on);
	...
	ret = psee_graph_entity_start_stop_subdev(pdev, entity, on);
	if (ret < 0) {
		dev_err(pdev->dev, "ret = %d for entity %s\n",
...
/**
 * psee_graph_pipeline_start_stop - start or stop the pipe in the graph
 * @pdev: composite device
 ...
 * psee_graph_entity_start_stop() takes care of dependencies,
 * or state-checking.
 *
				   ...
				   struct psee_pipeline *pipe, bool on)
{
	...
	list_for_each_entry(asd, &pdev->notifier.asd_list, asd_list) {
		...
		state = psee_graph_entity_start_stop(pdev, entity, on);
		if (!state)
			return -EPIPE;
	}
